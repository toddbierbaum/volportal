<x-layouts.admin :title="'Event templates · Admin'">
    <div class="flex items-start justify-between gap-4 mb-6 flex-wrap">
        <div class="max-w-2xl">
            <h1 class="text-2xl font-semibold text-gray-900">Event templates</h1>
            <p class="text-sm text-gray-500 mt-1">Each template captures the name, color, default positions, and default reminders for a kind of event (Kids Production, Monthly Show, etc.). Creating an event from a template auto-populates all of it.</p>
        </div>
            ...
            <ul class="divide-y divide-gray-100">
                @foreach ($templates as $template)
                    ...
                    <li class="px-5 py-4 flex items-center justify-between gap-4 hover:bg-gray-50 transition">
                        ...
                        <a href="{{ route('admin.event-templates.edit', $template) }}" class="text-sm text-fct-navy hover:underline shrink-0">Edit</a>
                    </li>
                @endforeach
            </ul>
        @endif
