<x-layouts.admin :title="'Events · Admin'">
    <div class="flex items-start justify-between gap-4 mb-6 flex-wrap">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Create and manage theater events.</p>
        </div>
    ...
    <section class="bg-white rounded-lg border border-gray-200 mb-6">
        ...
        @if ($upcoming->isEmpty())
            <div class="p-8 text-sm text-gray-500 text-center">No upcoming events.</div>
        @else
    ...
    @if ($past->isNotEmpty())
        <section class="bg-white rounded-lg border border-gray-200">
            <div class="px-5 py-4 border-b border-gray-100">
                <h2 class="text-base font-semibold text-gray-900">Past <span class="text-sm font-normal text-gray-500">· recent 25</span></h2>
            </div>
            <ul class="divide-y divide-gray-100">
                @foreach ($past as $event)
                    @php $color = $event->template?->color ?? '#9CA3AF'; @endphp
                    <li class="px-5 py-4 flex items-center gap-3 hover:bg-gray-50 transition">
                        <span class="inline-block h-2 w-2 rounded-full shrink-0 opacity-60" style="background-color: {{ $color }}"></span>
                        <a href="{{ route('admin.events.edit', $event) }}"
                           class="font-medium text-gray-900 hover:text-fct-navy">{{ $event->title }}</a>
                        <div class="text-sm text-gray-500 ml-auto">{{ $event->starts_at->format('D, M j Y') }}</div>
                    </li>
                @endforeach
            </ul>
        </section>
