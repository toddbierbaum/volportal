<x-layouts.admin :title="'Edit ' . $event->title . ' · Admin'">
    <a href="{{ route('admin.events.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-fct-navy dark:text-fct-cyan mb-4">
    ...
    <div class="mb-6 flex items-start justify-between gap-4 flex-wrap">
            ...
            <div class="min-w-0">
                ...
                @if ($event->is_published)
                    <p class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-1">
                        Public page:
                        <a href="{{ route('events.show', $event->slug) }}" target="_blank" class="text-fct-navy dark:text-fct-cyan hover:underline">
        ...
        <div class="flex items-center gap-2 shrink-0">
            <form method="POST" action="{{ route('admin.events.duplicate', $event) }}" class="inline">
                @csrf
                <button type="submit" class="px-3 py-2 text-sm rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:bg-gray-800/50 text-gray-700 dark:text-gray-300">Duplicate</button>
            </form>
            <form method="POST" action="{{ route('admin.events.destroy', $event) }}"
                ...
                @csrf
                @method('DELETE')
                <button type="submit" class="px-3 py-2 text-sm rounded-md border border-red-200 dark:border-red-900/60 text-red-700 dark:text-red-400 bg-white dark:bg-gray-800 hover:bg-red-50 dark:bg-red-900/30">Delete</button>
            </form>
        </div>
    </div>
