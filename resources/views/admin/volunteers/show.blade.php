<x-layouts.admin :title="$volunteer->name . ' · Admin'">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 mb-6">
        <div class="flex items-start justify-between gap-4 flex-wrap">
            <div class="flex items-center gap-4 min-w-0">
                ...
                <div class="min-w-0">
                    ...
                    <div class="mt-1 text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500 space-y-0.5">
            ...
            <div class="text-right">
                <div class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider font-medium">Lifetime hours</div>
        ...
        <div class="mt-6 pt-4 border-t border-gray-100 dark:border-gray-700/60">
            <form method="POST" action="{{ route('admin.volunteers.destroy', $volunteer) }}"
                ...
                @csrf
                @method('DELETE')
                <button type="submit" class="text-sm text-red-600 dark:text-red-400 hover:text-red-700 dark:text-red-400">Delete volunteer</button>
            </form>
        </div>
    </div>
    ...
    <section class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 mb-6">
        ...
        @if ($upcomingSignups->isEmpty())
            <div class="p-8 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 text-center">No upcoming signups.</div>
        @else
                ...
                @foreach ($upcomingSignups as $signup)
                    ...
                    <li class="px-5 py-4 flex items-center justify-between gap-4 hover:bg-gray-50 dark:bg-gray-800/50 transition">
                        <div class="flex items-center gap-3 min-w-0">
                            ...
                            <div class="min-w-0">
                                <a href="{{ route('admin.events.edit', $signup->position->event) }}" class="font-medium text-gray-900 dark:text-gray-100 hover:text-fct-navy dark:text-fct-cyan">
                        ...
                        @elseif ($signup->status === 'cancelled' || $signup->status === 'canceled')
                            <span class="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 dark:text-gray-500 font-medium shrink-0">Cancelled</span>
                        @else
        ...
        <section class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
            ...
            <ul class="divide-y divide-gray-100 dark:divide-gray-700/60">
                @foreach ($pastSignups as $signup)
                    @php $color = $signup->position->event->template?->color ?? '#9CA3AF'; @endphp
                    <li class="px-5 py-4 flex items-center justify-between gap-4 text-sm hover:bg-gray-50 dark:bg-gray-800/50 transition">
                        <div class="flex items-center gap-3 min-w-0">
                        ...
                        <span class="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 dark:text-gray-500 font-medium shrink-0">{{ ucfirst(str_replace('_', ' ', $signup->status)) }}</span>
                    </li>
                @endforeach
            </ul>
        </section>
