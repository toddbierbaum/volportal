<x-layouts.admin :title="'Volunteers · Admin'">
    <div class="flex items-start justify-between gap-4 mb-6 flex-wrap">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">Volunteers</h1>
            <p class="mt-1 text-sm text-gray-500">Hours reporting and contact info.</p>
        </div>
    ...
    <form method="GET" class="mb-4 bg-white border border-gray-200 rounded-lg p-4 space-y-3">
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div class="sm:col-span-2">
                <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider">Search</label>
                <input type="search" name="q" value="{{ $q }}"
                       placeholder="Name, email, or phone…"
                       class="mt-1 block w-full border-gray-300 focus:border-fct-cyan focus:ring-fct-cyan rounded-md text-sm">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider">Hours from</label>
                <input type="date" name="from" value="{{ $from }}"
                       class="mt-1 block w-full border-gray-300 focus:border-fct-cyan focus:ring-fct-cyan rounded-md text-sm">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider">Through</label>
        ...
        <div class="flex items-center justify-between flex-wrap gap-2">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('admin.volunteers.export', request()->query()) }}"
                   class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-md border border-gray-300 bg-white hover:bg-gray-50 text-gray-700">
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="mb-6 bg-gradient-to-r from-fct-cyan/10 to-fct-cyan/5 border border-fct-cyan/30 rounded-lg p-5 flex items-center justify-between gap-4 flex-wrap">
        <div>
            <div class="text-xs text-fct-navy uppercase tracking-wider font-semibold">
                Total hours {{ $from || $to ? '(filtered)' : '(lifetime)' }}
            </div>
            <div class="text-3xl font-semibold text-fct-navy mt-1">{{ number_format((float) $totalHours, 2) }}</div>
        </div>
        @if ($from || $to)
                    ...
                    <li class="px-5 py-4 flex items-center justify-between gap-4 hover:bg-gray-50 transition">
                        ...
                        <div class="text-sm text-right shrink-0">
                            <div class="font-semibold text-gray-900">
                                ...
                                <span class="text-xs font-normal text-gray-500">hrs</span>
                            </div>
