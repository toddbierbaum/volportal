<div>
    @if ($flash)
        <div class="mb-4 px-4 py-3 rounded-md bg-emerald-50 border border-emerald-200 text-sm text-emerald-900">
            {{ $flash }}
        </div>
    @endif
    ...
    <div class="bg-white rounded-lg border border-gray-200 mb-6">
        @if ($items->isEmpty())
            <div class="p-8 text-center text-sm text-gray-500">No categories yet.</div>
        @else
        ...
        <div class="px-5 py-4 border-t border-gray-100 bg-gray-50 rounded-b-lg">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-medium text-gray-600 uppercase tracking-wider">Name</label>
                    <input type="text" wire:model="name"
                           class="mt-1 block w-full border-gray-300 focus:border-fct-cyan focus:ring-fct-cyan rounded-md text-sm">
                    @error('name') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 uppercase tracking-wider">Color</label>
                    <input type="color" wire:model="color"
                           class="mt-1 block h-9 w-24 border border-gray-300 rounded-md">
                    @error('color') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
                <div class="sm:col-span-2">
                    <label class="block text-xs font-medium text-gray-600 uppercase tracking-wider">Description (optional)</label>
