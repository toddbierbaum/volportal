<div>
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
        <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60">
            <h2 class="text-base font-semibold text-gray-900 dark:text-gray-100">Volunteer signups</h2>
        ...
        @if ($positions->isEmpty())
            <div class="p-8 text-center text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">No positions on this event yet — add some above.</div>
        @else
                ...
                @foreach ($positions as $position)
                    ...
                    <div class="px-5 py-4">
                        <div class="flex items-center justify-between gap-4 flex-wrap">
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <span class="font-semibold text-gray-900 dark:text-gray-100">{{ $position->title }}</span>
                            ...
                            @if ($assigningForPositionId !== $position->id)
                                <button type="button" wire:click="startAssigning({{ $position->id }})"
                                        class="inline-flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:bg-gray-800/50 text-gray-700 dark:text-gray-300">
                        ...
                        @if ($nonCancelled->isEmpty())
                            <div class="mt-3 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">No signups yet.</div>
                        @else
                            <ul class="mt-3 divide-y divide-gray-100 dark:divide-gray-700/60 border border-gray-200 dark:border-gray-700 rounded-md overflow-hidden">
                                @foreach ($nonCancelled as $signup)
                                    <li class="p-3 flex items-center justify-between gap-3 flex-wrap bg-white dark:bg-gray-800 hover:bg-gray-50 dark:bg-gray-800/50 transition">
                                        <div class="flex items-center gap-3 min-w-0">
