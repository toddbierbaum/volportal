<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sm:p-8">
        ...
        {{-- Step 1: contact info --}}
        @if ($step === 1)
            <h2 class="text-2xl font-semibold text-fct-navy mb-1">Tell us about yourself</h2>
            <p class="text-sm text-gray-500 mb-6">We'll use this to match you with opportunities and send reminders.</p>
        ...
        {{-- Step 2: categories --}}
        @if ($step === 2)
            <h2 class="text-2xl font-semibold text-fct-navy mb-1">How would you like to help?</h2>
            <p class="text-sm text-gray-500 mb-6">Pick the areas you're interested in. You can change these anytime.</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                @foreach ($categories as $category)
                    ...
                    <label class="relative flex items-start p-4 border-2 rounded-lg cursor-pointer transition
                        {{ in_array($category->id, $selectedCategoryIds) ? 'border-fct-navy bg-fct-cyan/5' : 'border-gray-200 hover:border-gray-300' }}">
        ...
        {{-- Step 3: matches --}}
        @if ($step === 3)
            <h2 class="text-2xl font-semibold text-fct-navy mb-1">Opportunities that match your interests</h2>
            ...
            @if ($matchedPositions->isNotEmpty())
                <ul class="divide-y divide-gray-100 border border-gray-200 rounded-lg overflow-hidden">
                    @foreach ($matchedPositions as $position)
                        ...
                        @endphp
                        <li class="p-4 flex items-center justify-between gap-4 hover:bg-gray-50 transition">
                            <div class="flex items-start gap-3 min-w-0">
                                <span class="inline-block h-2.5 w-2.5 rounded-full shrink-0 mt-1.5" style="background-color: {{ $color }}"></span>
                                <div class="min-w-0">
                                    <div class="text-xs text-gray-500 uppercase tracking-wider font-medium">{{ $position->event->starts_at->format('D, M j · g:i A') }}</div>
                                    <div class="font-medium text-gray-900 mt-0.5">{{ $position->event->title }}</div>
                            ...
                            <div class="shrink-0">
                                @if ($isSignedUp)
                                    <span class="inline-flex items-center gap-1 px-3 py-1.5 text-sm rounded-md bg-emerald-50 text-emerald-700 font-medium">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @else
                                    <button type="button" wire:click="signUp({{ $position->id }})"
                                            ...
                                            class="inline-flex items-center px-4 py-2 text-sm rounded-md font-medium transition
                                                {{ $isFull ? 'bg-amber-50 text-amber-800 hover:bg-amber-100' : 'bg-fct-navy text-white hover:bg-fct-navy-light' }}">
        ...
        @if ($step === 5)
            <div class="text-center py-4">
                <div class="mx-auto flex items-center justify-center h-14 w-14 rounded-full bg-fct-cyan/20">
                    <svg class="h-7 w-7 text-fct-navy" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                    </svg>
                </div>
                <h2 class="mt-4 text-2xl font-semibold text-fct-navy">Welcome back</h2>
                ...
                <p class="mt-4 text-sm text-gray-500">
                    Not your account?
                    <button type="button" wire:click="backToDetails" class="text-fct-navy underline hover:text-fct-navy-light">Use a different email</button>.
                </p>
            </div>
        @endif
        ...
        @if ($step === 4)
            <div class="text-center py-4">
                ...
                <h2 class="mt-4 text-2xl font-semibold text-fct-navy">Thanks, {{ explode(' ', $name)[0] ?: 'volunteer' }}!</h2>

                @if ($createdSignups->isNotEmpty())
                    <p class="mt-2 text-sm text-gray-600">You signed up for:</p>
                    <ul class="mt-4 text-left max-w-md mx-auto divide-y divide-gray-100 border border-gray-200 rounded-lg overflow-hidden">
                                ...
                                <div class="flex items-center justify-between gap-2">
                                    <div class="flex items-start gap-3 min-w-0">
                                        ...
                                        <div class="min-w-0">
                                            ...
                                            <div class="text-sm text-gray-500">{{ $signup->position->title }} &middot; {{ $signup->position->event->starts_at->format('D, M j · g:i A') }}</div>
                                        </div>
                                    </div>
                ...
                @else
                    <p class="mt-2 text-sm text-gray-600 max-w-md mx-auto">Your interests are saved. We'll email you when matching opportunities come up.</p>
                @endif
