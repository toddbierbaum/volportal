...
- Confirm session storage does not rely on a shared `/tmp` directory. For VolPortal this means `SESSION_DRIVER=database` (already set).
- Verify scheduled commands are wired through DreamHost's cron panel with the correct PHP binary path, and their output is captured to a log file outside the webroot.
- Confirm DreamHost's `phprc` file (if used) does not re-enable dangerous functions (`exec`, `shell_exec`, `passthru`) beyond what the app requires.
